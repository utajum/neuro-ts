...
async function main(): Promise<void> {
  console.log('[neuro-js] generating wrappers...');
  const libFiles = listLibFiles();
  console.log(`[neuro-js] loaded ${libFiles.length} lib.*.d.ts files`);
  ...
  if (missing.length > 0)
    console.warn(`[neuro-js] interface symbols not found: ${missing.join(', ')}`);
  ...
  for (const spec of BUILTINS) {
    ...
    console.log(`[neuro-js] ${spec.interface} -> neuro.${spec.group} (+${collected.length})`);
  }
  ...
  for (const [name, decl] of globalDecls) {
    ...
    if (cm) {
      globalsList.push(cm);
      console.log(`[neuro-js] global ${name} -> neuro.${name}`);
    }
  }
  ...
  if (missingCurated.length > 0 || orphanCurated.length > 0) {
    ...
    const summary =
      `[neuro-js] curated prompt coverage gap:\n` +
...
`;
  ...
  console.log(`[neuro-js] emitted ${totalMethods} method(s) across ${groupNames.length} group(s)`);
}

main().catch((err) => {
  console.error('[neuro-js] generation failed:', err);
  process.exit(1);
});
