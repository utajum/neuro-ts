...
export const regExpPrompts: Record<string, CuratedPrompt> = {
  compile: {
    prompt:
      'recompile the regex with pattern and flags, mutating in place, the legacy entry that survives because removing it would break exactly one Rails app',
  ...
  exec: {
    prompt:
      'execute the regex on string, returning the next match array or null, advancing lastIndex when /g is set so consecutive calls walk the string',
    comment:
  ...
  test: {
    prompt:
      'return true if the regex matches string, advancing lastIndex when /g is set, the gotcha that breaks every loop the second time around',
