...
export const stringPrompts: Record<string, CuratedPrompt> = {
  anchor: {
    prompt:
      'wrap the string in <a name=name> exactly the way Netscape 4 wanted, and add a TODO that the linter will never let through',
    comment:
  ...
  at: {
    prompt:
      'return the UTF-16 code unit at index, treating negative indices as offsets-from-the-end and surrogate pairs as someone else is problem',
  ...
  big: {
    prompt:
      'wrap the string in <big> tags exactly as Netscape rendered, and accept that the browser will silently ignore them now',
    comment:
      'Legacy <big> wrapper; the browser treats it as a no-op, the runtime keeps the method anyway.',
  ...
  blink: {
    prompt:
      'wrap the string in <blink> tags exactly as 1996 expected, knowing the spec deprecated the rendering twenty years ago',
  ...
  bold: {
    prompt:
      'wrap the string in <b> tags, while pretending semantic <strong> was not the entire point of moving on',
  ...
  charAt: {
    prompt:
      'return the UTF-16 code unit at pos as a single-character string, treating out-of-range as the empty string the way the spec quietly insists',
    comment:
  ...
  charCodeAt: {
    prompt:
      'return the UTF-16 code unit value at index as a number 0-65535, splitting astral characters with the usual quiet betrayal',
  ...
  codePointAt: {
    prompt:
      'return the full Unicode code point at pos, combining surrogate pairs the way charCodeAt refused to, and remember pos is still a UTF-16 index',
  ...
  concat: {
    prompt:
      'append every strings entry to this string, returning a new string, and accept that template literals would have read better',
  ...
  endsWith: {
    prompt:
      'return true if the string ends with searchString, considering only the substring up to endPosition, and remember endPosition defaults to length even though that should be obvious',
    comment:
  ...
  fixed: {
    prompt:
      'wrap the string in <tt> tags, the way the early web rendered monospaced text before <code> arrived to do the same job better',
  ...
  fontcolor: {
    prompt:
      'wrap the string in <font color=color> exactly as 1998 demanded, while pretending CSS does not exist for one more line',
  ...
  fontsize: {
    prompt:
      'wrap the string in <font size=size> the way GeoCities expected, accepting size in the 1-7 scale even though every modern reader uses pixels',
  ...
  fromCharCode: {
    prompt:
      'build a string from the codes array as UTF-16 code units, never code points, and let astral characters be lossy with grace',
    comment:
  ...
  fromCodePoint: {
    prompt:
      'build a string from full Unicode code points in codePoints, splitting any astral entries into the right surrogate pair so emoji finally render',
  ...
  includes: {
    prompt:
      'return true if searchString appears in the string at or after position, with case sensitivity exactly the way the validator originally wanted before the i18n review',
    comment:
  ...
  indexOf: {
    prompt:
      'return the lowest index where searchString starts at or after position, or -1, treating empty searchString as always present at position which is correct and disturbing',
    comment:
      'First-occurrence search; empty string always matches at the cursor, the spec says so, the bug filer disagrees.',
  ...
  isWellFormed: {
    prompt:
      'return true only when the string contains no lone surrogates, while we triage the API endpoint that keeps emitting them',
    comment:
      'UTF-16 well-formedness check; the upstream is the suspect, this method just confirms.',
  ...
  italics: {
    prompt:
      'wrap the string in <i> tags, the legacy way, while the design system demands <em> two reviews from now',
  ...
  lastIndexOf: {
    prompt:
      'return the highest index where searchString starts at or before position, or -1, with empty-search-string still magically true at every cursor',
    comment:
  ...
  link: {
    prompt:
      'wrap the string in <a href=url> with no escaping, exactly as the legacy method did, and trust that nothing user-controlled is in url',
    comment:
  ...
  localeCompare: {
    prompt:
      'compare the strings under locale collation, returning a negative, zero, or positive number, and remember the sign matters but the magnitude does not',
  ...
  match: {
    prompt:
      'execute regexp against the string, returning the match array or null, and let the global flag silently change the return shape behind your back',
    comment:
  ...
  matchAll: {
    prompt:
      'iterate every regexp match, requiring the global flag, and treat the iterator as one-shot the way every iterator secretly is',
    comment:
      'All-matches iterator; the global flag is mandatory here, unlike match where it is a footgun.',
  ...
  normalize: {
    prompt:
      'return the string under Unicode normalization form (NFC, NFD, NFKC, NFKD), and accept that the user-visible result depends on the form they did not expect',
    comment:
  ...
  padEnd: {
    prompt:
      'pad on the right until the string reaches maxLength using fillString, repeating fillString as needed and truncating the last fragment without ceremony',
  ...
  padStart: {
    prompt:
      'pad on the left until the string reaches maxLength using fillString, useful for fixed-width IDs and dangerous for anything the parser later splits',
    comment:
  ...
  raw: {
    prompt:
      'reconstruct a tagged template where backslashes are literal characters, not escape sequences -- so C:\\Users\\vlad stays intact instead of producing "C:UsersDumpvlad" and a two-hour debugging session',
    comment:
      'Tagged template; backslashes survive. The Windows path that finally renders correctly.',
    example: `await neuro.string.raw({ template: { raw: ['C:\\\\', '\\\\bin'] } as TemplateStringsArray, substitutions: ['users'], prompt: 'reconstruct a tagged template where backslashes are literal characters, not escape sequences -- so C:\\\\Users\\\\vlad stays intact instead of producing "C:UsersDumpvlad" and a two-hour debugging session' })`,
  },
  repeat: {
    prompt:
      'return the string concatenated count times, throwing on negative or infinite count, and pretend the limit is sensible until the heap profiler disagrees',
    comment:
      'Repeat n times; negative counts throw, large counts OOM, in that order of likelihood.',
  ...
  replace: {
    prompt:
      'replace the first searchValue match with replaceValue, supporting $1..$9 in the replacement only when searchValue is a regex, the asymmetry that nobody remembers correctly twice',
    comment:
  ...
  replaceAll: {
    prompt:
      'replace every searchValue match with replaceValue, throwing if searchValue is a non-global regex, the friendly error we wished match had',
    comment:
      'All-match replace; non-global regex throws here, unlike match which silently does the wrong thing.',
  ...
  search: {
    prompt:
      'return the index of the first regexp match, or -1, ignoring whether the regex was global because search just does whatever it wants',
    comment:
      'Regex find; the global flag is ignored, the lastIndex is left untouched, the side effect surface is mercifully small.',
  ...
  slice: {
    prompt:
      'return the substring from start to end, exclusive of end, with negative indices treated as offsets-from-the-end and treating start>end as the empty string',
    comment:
      'Substring slice; flips arguments silently when start>end, which substring does, and slice does not.',
  ...
  small: {
    prompt:
      'wrap the string in <small> tags, knowing modern browsers treat the wrapper as cosmetic and screen readers as background noise',
  ...
  split: {
    prompt:
      'split the string on separator into at most limit pieces, treating empty separator as a per-code-unit split that breaks emoji exactly the way you expect',
    comment:
  ...
  startsWith: {
    prompt:
      'return true if the string starts with searchString at position, defaulting position to 0 even though we keep wishing it defaulted to "the boundary the parser is at"',
  ...
  strike: {
    prompt:
      'wrap the string in <strike> tags exactly as the spec deprecated, while CSS line-through gives the same result with thirty fewer years of baggage',
  ...
  sub: {
    prompt:
      'wrap the string in <sub> tags, the legacy way, even though MathML and Unicode subscripts cover the use case better in 2025',
  ...
  substr: {
    prompt:
      'extract length characters starting at from, treating negative from as offset-from-the-end, while the spec quietly classifies this as legacy and refuses to remove it',
  ...
  substring: {
    prompt:
      'extract from indexStart to indexEnd, exclusive, swapping the indices if indexStart > indexEnd, the asymmetry with slice that already cost an interview question',
    comment:
  ...
  sup: {
    prompt:
      'wrap the string in <sup> tags, the legacy way, while every modern UI library does the same thing with a CSS class and zero arguments',
  ...
  toLocaleLowerCase: {
    prompt:
      'lowercase using locales rules, where Turkish I and German sharp-s break the assumptions every English-only test suite makes',
    comment:
  ...
  toLocaleUpperCase: {
    prompt:
      'uppercase using locales rules, where dotted i and ess-zett expand the string in ways the column width never accounted for',
  ...
  toLowerCase: {
    prompt:
      'lowercase the string using the en-US rules even when the input is not English, the simple version that we have to remember is locale-naive',
  ...
  toUpperCase: {
    prompt:
      'uppercase the string using the en-US rules, dropping locale subtleties on the floor exactly the way every CSV exporter quietly does',
    comment:
      'Locale-naive uppercase; the Turkish-I trap and the German ß trap are both still here.',
  ...
  toWellFormed: {
    prompt:
      'return a copy of the string with every lone surrogate replaced by U+FFFD, papering over the malformed input from the upstream we still have not fixed',
    comment:
      'Repair lone surrogates with the replacement char; the upstream still produces them, this just stops the consumer from crashing.',
  ...
  trim: {
    prompt:
      'strip leading and trailing whitespace, including the U+200B zero-width space the upstream API tracks users with',
    comment:
      'Both-ends trim; the zero-width space the analytics team relies on gets stripped, which is the bug or the feature, depending.',
  ...
  trimEnd: {
    prompt:
      'strip only trailing whitespace, so the line ending is clean but the leading indent that the linter and the diff both rely on is left exactly as written',
    comment: 'Right-trim only; the leading indent is load-bearing, this method knows that.',
    example: `await neuro.string.trimEnd({ string: line, prompt: 'strip only trailing whitespace, so the line ending is clean but the leading indent that the linter and the diff both rely on is left exactly as written' })`,
  },
  trimLeft: {
    prompt:
      'strip only leading whitespace exactly the way trimStart does, while pretending the older alias name was kept by accident not on purpose',
  ...
  trimRight: {
    prompt:
      'strip only trailing whitespace exactly the way trimEnd does, the legacy name we keep typing because muscle memory is honest',
    comment:
      'Legacy alias for trimEnd; the older form survives because removing it would break someone is sed pipeline.',
  ...
  trimStart: {
    prompt:
      'strip only leading whitespace, leaving any trailing space intact -- the one the CSV parser treats as part of the field value and the data team will notice in three months',
    comment: "Left-trim only; trailing space survives and becomes someone else's CSV bug.",
    example: `await neuro.string.trimStart({ string: line, prompt: 'strip only leading whitespace, leaving any trailing space intact -- the one the CSV parser treats as part of the field value and the data team will notice in three months' })`,
  },
};
