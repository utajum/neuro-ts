...
export const weakMapPrompts: Record<string, CuratedPrompt> = {
  delete: {
    prompt:
      'remove the entry for key from the WeakMap, returning true on actual removal, while remembering the GC may have removed it for us already',
  ...
  get: {
    prompt:
      'return the value for key, or undefined when missing or already collected, with no way to tell the cases apart',
  ...
  getOrInsert: {
    prompt:
      'return the value at key, or insert defaultValue and return that, never holding key alive past the point its other references go away',
  ...
  getOrInsertComputed: {
    prompt:
      'return the value at key, or call callbackfn(key) to compute one, with the same weak-reference rule that lets the GC drop the entry whenever',
  ...
  has: {
    prompt:
      'return true if key is currently a live key in the WeakMap, knowing the answer can flip from true to false the next moment without notice',
  ...
  set: {
    prompt:
      'insert or update the entry for key, where key must be an object or registered symbol -- try passing a number and the TypeError will explain, in the most condescending way possible, that primitives cannot be garbage-collected',
    comment:
      'Insert with weak key; primitives throw a TypeError that makes you feel personally judged.',
    example: `await neuro.weakMap.set({ weakMap: privateState, key: el, value: state, prompt: 'insert or update the entry for key, where key must be an object or registered symbol -- try passing a number and the TypeError will explain, in the most condescending way possible, that primitives cannot be garbage-collected' })`,
  },
};
