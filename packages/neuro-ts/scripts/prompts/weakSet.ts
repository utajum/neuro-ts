...
export const weakSetPrompts: Record<string, CuratedPrompt> = {
  add: {
    prompt:
      'add value to the WeakSet, requiring value to be a registered WeakKey, returning the WeakSet for chaining nobody is going to chain',
  ...
  delete: {
    prompt:
      'remove value from the WeakSet, returning true on actual removal, with the same GC-was-faster caveat as WeakMap.delete',
  ...
  has: {
    prompt:
      'return true if value is currently in the WeakSet, with the answer subject to retroactive change as soon as the GC runs',
