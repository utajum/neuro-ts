...
// Per-method routing & error-path tests for the `bigInt` group.
...
const cases: MethodCase[] = [
  {
    ...
    routedInput: { bits: undefined, int: undefined, prompt: 'test' },
    nativeInput: { bits: undefined, int: undefined },
    invoke: (input) => (neuro.bigInt.asIntN as (i: Record<string, unknown>) => Promise<unknown>)(input),
  },
  {
    name: 'asUintN',
    ...
    routedInput: { bits: undefined, int: undefined, prompt: 'test' },
    nativeInput: { bits: undefined, int: undefined },
    invoke: (input) => (neuro.bigInt.asUintN as (i: Record<string, unknown>) => Promise<unknown>)(input),
  }
];
...
describe('bigInt native values', () => {
  test('asIntN native fallback matches native built-in', async () => {
    const result = await (neuro.bigInt.asIntN as (i: Record<string, unknown>) => Promise<unknown>)({ bits: 4, int: 10n });
    expect(result).toBe(BigInt.asIntN(4, 10n));
  });

  test('asUintN native fallback matches native built-in', async () => {
    const result = await (neuro.bigInt.asUintN as (i: Record<string, unknown>) => Promise<unknown>)({ bits: 4, int: 10n });
    expect(result).toBe(BigInt.asUintN(4, 10n));
  });
});
