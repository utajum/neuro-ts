...
// Per-method routing & error-path tests for the `error` group.
...
const cases: MethodCase[] = [
  {
    ...
    routedInput: { error: undefined, prompt: 'test' },
    nativeInput: { error: undefined },
    invoke: (input) => (neuro.error.isError as (i: Record<string, unknown>) => Promise<unknown>)(input),
  }
];
...
describe('error native values', () => {
  test.skip('isError native fallback does not throw', async () => { // group 'error' not handled
    await expect((neuro.error.isError as (i: Record<string, unknown>) => Promise<unknown>)({})).resolves.not.toThrow();
  });
});
