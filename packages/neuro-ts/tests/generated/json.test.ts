...
// Per-method routing & error-path tests for the `json` group.
...
const cases: MethodCase[] = [
  {
    ...
    routedInput: { text: undefined, reviver: undefined, prompt: 'test' },
    nativeInput: { text: undefined, reviver: undefined },
    invoke: (input) => (neuro.json.parse as (i: Record<string, unknown>) => Promise<unknown>)(input),
  },
  {
    name: 'stringify',
    functionId: 'JSON.stringify',
    routedInput: { value: undefined, replacer: undefined, space: undefined, prompt: 'test' },
    nativeInput: { value: undefined, replacer: undefined, space: undefined },
    invoke: (input) => (neuro.json.stringify as (i: Record<string, unknown>) => Promise<unknown>)(input),
  }
];
...
describe('json native values', () => {
  test('parse native fallback matches native built-in (deep)', async () => {
    const result = await (neuro.json.parse as (i: Record<string, unknown>) => Promise<unknown>)({ text: '{"a":1}' });
    expect(result).toEqual(JSON.parse('{"a":1}'));
  });

  test('stringify native fallback matches native built-in', async () => {
    const result = await (neuro.json.stringify as (i: Record<string, unknown>) => Promise<unknown>)({ value: { a: 1 } });
