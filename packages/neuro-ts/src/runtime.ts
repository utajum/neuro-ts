...
function extractPrompt(input: Record<string, unknown>): {
  ...
  if (input == null || typeof input !== 'object') {
    throw new NeuroClientError(
      'neuro-ts wrappers expect a single object argument, e.g. neuro.array.map({ array, callbackfn, prompt? }).',
    );
  }
...
function runNative(spec: GeneratedMethodSpec, rest: Record<string, unknown>): unknown {
  if (!spec.native) {
    throw new NeuroClientError(
      `\`${spec.functionId}\` cannot run natively from neuro-ts. Provide a \`prompt\` field to route through the LLM.`,
    );
  }
  ...
  // Static methods on constructors (Promise, TypedArrays, etc.) require the
  // constructor itself as `this` (e.g. `Promise.all`, `BigInt64Array.from`).
  // Resolve the parent object from functionId so these work without modifying
  // the generated spec files.
  const nativeThis = resolveNativeParent(spec.functionId);
  return spec.native.apply(nativeThis, positional);
}
...
/**
 * Resolve the parent object of a static method so it can be used as `this`
 * when invoking via `apply`. For `Promise.all` returns `Promise`; for
 * `Math.random` returns `Math`; for `globalThis.parseInt` returns `globalThis`.
 * For prototype methods (e.g. `Array.prototype.map`) returns `undefined` since
 * those are called with the instance as receiver via `apply(receiver, ...)`.
 */
function resolveNativeParent(functionId: string): unknown {
  const parts = functionId.split('.');
  if (parts.length < 2) return undefined;
  // Prototype methods use the instance as `this` - no parent needed.
  if (parts.includes('prototype')) return undefined;
