...
export class Animator<
  const T extends Record<string, AnimationConfig<unknown>>,
> {
  private readonly configs: T;
  private animations: Record<
    keyof T,
    AnimationState<ConfigValueType<T[keyof T]>>
  >;
  private onMultipleFinished: {
    keys: (keyof T)[];
    onFinished: (values: ConfigValueType<T[keyof T]>[]) => void;
  }[] = [];
  ...
  startAnimationTo<K extends keyof T, P extends keyof T[K]["properties"]>(
    ...
    this.startAnimation(
      key,
      this.configs[key]!.properties![property as string] as ConfigValueType<
        T[K]
      >,
      options,
    );
  ...
  startAnimation<K extends keyof T>(
    ...
    animation.sourceValue = config.interpolator.clone(
  ...
  setProperties<K extends keyof T>(
    key: K,
    properties: AnimationToMap<T>,
  ): void {
  ...
  setEasing<K extends keyof T>(
    key: K,
    easing: AnimationConfig<unknown>["easing"],
  ): void {
    this.configs[key]!.easing = easing;
  }

  setInterpolator<K extends keyof T>(
    key: K,
    interpolator: AnimationConfig<unknown>["interpolator"],
  ): void {
  ...
  update(dt: number): void {
    ...
    for (const { keys, onFinished } of this.onMultipleFinished) {
      if (!this.isAnimatingMultiple(keys)) {
        ...
        this.onMultipleFinished = this.onMultipleFinished.filter(
          (x) => x.onFinished !== onFinished,
        );
      }
    }
  }

  setOnFinished<K extends keyof T>(
    key: K,
