...
export class GraphSimulator {
    ...
    this.simulation
      ...
      .force(
        ...
        d3
          ...
          .distanceMax(500)
          .strength(-this.context.config.repelForce),
      )
      .force(
        "forceX",
        d3.forceX<NodeData>().strength(this.context.config.centerForce),
      )
      .force(
        "forceY",
        d3.forceY<NodeData>().strength(this.context.config.centerForce),
      )
      .force(
        "collision",
        d3
          ...
          .radius(
            (node) => node.colliderSize! + this.context.config.colliderPadding,
          ),
      )
  ...
  enableDrag() {
    ...
    d3.select(this.container).call(
      (
        d3.drag().container(this.container) as unknown as d3.DragBehavior<
          HTMLCanvasElement,
          unknown,
          unknown
        >
  ...
  enableHover() {
    d3.select(this.container).on("mousemove", (e: MouseEvent) => {
      ...
      if (closestNode) {
        ...
        if (
          this.context.config.prefetchPages &&
          closestNode !== this.currentNode &&
          !closestNode.external
        ) {
        ...
        this.container.style.cursor =
          this.context.enableClick && this.isClickable(closestNode)
            ? "pointer"
            : "default";
    ...
    d3.select(this.container).on("click", (e: MouseEvent) => {
      ...
      if (closestNode && this.isClickable(closestNode)) {
        ...
        if (
          ...
          (clickTime - this.lastClick < 500 &&
            closestNode === this.lastClickedNode)
        ) {
          if (closestNode.external) {
            ...
            window.open(
              ...
              this.context.config.followLink === "new-tab" ? "_blank" : "_self",
            );
          }
        }
      ...
      (this.zoomBehavior = (
        ...
        .on("zoom", ({ transform }: { transform: d3.ZoomTransform }) => {
          ...
          if (!this.context.config.enablePan) {
            ...
            this.zoomTransform = new d3.ZoomTransform(
              transform.k,
              cx * (1 - transform.k / this.scale),
              cy * (1 - transform.k / this.scale),
            );
          } else {
  ...
  isClickable(node: NodeData): boolean {
    return (
      node.exists && !(node.type === "tag" || node.id === this.currentNode?.id)
    );
  }
  ...
  updateCenterTransform(): boolean {
    ...
    if (this.currentNode) {
      x =
        this.container.clientWidth / (2 * this.scale) -
        this.scale * this.currentNode.x!;
      y =
        this.container.clientHeight / (2 * this.scale) -
        this.scale * this.currentNode.y!;
    } else {
