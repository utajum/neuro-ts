...
export class GraphRenderer {
  ...
  tick(ticker: PIXI.Ticker) {
    ...
    this.linkHoverGraphics.alpha =
      this.context.animator.getValue("linkOpacityHover");
    this.arrowHoverGraphics.alpha =
      this.context.animator.getValue("linkOpacityHover");
  }
  ...
  drawNodeShape(node: NodeData, hovered?: boolean, adjacent?: boolean) {
    ...
    this.drawNode(
      ...
      hovered === undefined
        ? NODE_DEFAULT_Z_INDEX
        : hovered
          ? NODE_HOVER_Z_INDEX
          : NODE_MUTED_Z_INDEX;
    node.node!.tint = this.context.animator.getValue(
      (node.shapeColor +
        (hovered ? "Hover" : adjacent ? "Adjacent" : "")) as any,
    ) as string;
  ...
  drawNodeStroke(node: NodeData, hovered?: boolean, adjacent?: boolean) {
    ...
    if (node.strokeColor === "inherit") {
      ...
      strokeFill = 0xffffff;
      strokeTint = this.context.animator.getValue(
        (node.strokeColor +
          (hovered ? "Hover" : adjacent ? "Adjacent" : "")) as any,
      ) as string;
    }
    ...
    node.stroke!._zIndex =
      hovered === undefined
        ? STROKE_DEFAULT_Z_INDEX
        : hovered
          ? STROKE_HOVER_Z_INDEX
          : STROKE_MUTED_Z_INDEX;
    this.drawNode(
      node.stroke!,
      node.shape!,
  ...
  drawNodes(nodes: NodeData[]) {
    for (const node of nodes) {
      const hovered =
        ...
        node.id === this.simulator.currentlyHovered;
      let adjacent = false;
        ...
        x +
          radius *
            (t * Math.cos(node.shapeRotation! + segment * segmentAngle) +
              (1 - t) *
                Math.cos(node.shapeRotation! + (segment + 1) * segmentAngle)),
        y +
          radius *
            (t * Math.sin(node.shapeRotation! + segment * segmentAngle) +
              (1 - t) *
                Math.sin(node.shapeRotation! + (segment + 1) * segmentAngle)),
      ];
  ...
  drawLink(link: LinkData, hovered: boolean) {
    const linkZoomLevel = this.context.config.scaleLinks
      ? this.context.animator.getValue("zoom")
      : 1;
    const incAngle = Math.atan2(
    ...
    if (
      this.context.config.renderArrows &&
      this.simulator.zoomTransform.k > this.context.config.minZoomArrows
    ) {
  ...
  drawArrowHead(
    ...
    const arrowZoomLevel = this.context.config.scaleArrows
      ? this.context.animator.getValue("zoom")
      : 2;
    const x = nodeX;
    const y = nodeY;
    const arrowSize =
      (DEFAULT_ARROW_SCALE * (this.context.config.arrowSize + linkWidth)) /
      arrowZoomLevel;
    const xLeft =
  ...
  drawLinks(links: LinkData[]) {
    ...
    this.linkGraphics.clear().zIndex = hovered
      ? LINK_MUTED_Z_INDEX
      : LINK_DEFAULT_Z_INDEX;
    this.linkHoverGraphics.clear();
    this.arrowGraphics.clear().zIndex = hovered
      ? ARROW_MUTED_Z_INDEX
      : ARROW_DEFAULT_Z_INDEX;
  ...
  updateLabel(node: NodeData, hovered?: boolean, adjacent?: boolean) {
    ...
    if (hovered) {
      ...
      labelOpacity = this.context.animator.getValue(
        "labelOpacity" + (adjacent ? "Adjacent" : ""),
      );
