...
export class GraphComponent extends HTMLElement {
  ...
  constructor() {
    ...
    try {
      ...
      this.currentPage = setSlashes(
        this.dataset["slug"] || location.pathname,
        true,
      ...
      console.error(
        "[STARLIGHT-SITE-GRAPH] " + (e instanceof Error ? e.message : e),
      );
    }
    ...
    this.themeObserver = new MutationObserver(() => {
      ...
      this.animator.startAnimationsTo(this.defaultColorTransitions, {
        duration: 200,
      });
    });
  ...
  validateConfig(config: any) {
    ...
    config.depth =
      config.depth < 0 || config.depth >= MAX_DEPTH
        ? MAX_DEPTH - 1
        : config.depth;
  ...
  setupColors(usedColors: string[], customColorMap: Record<string, string>) {
    ...
    this.animator.setConfigs(
      animatables(this.config, this.colors, this.usedColors),
    );
    this.animator.startAnimationsTo(this.defaultColorTransitions, {
      duration: 200,
    });
  ...
  setup() {
    ...
    this.cleanup();
    const { nodes, links, usedColors, customColorMap } = processSitemapData(
      this,
      this.sitemap,
    );
  ...
  toggleFullscreen() {
    ...
    this.animator.setAllProperties(
      "backgroundColor",
  ...
  setStyleDefault() {
    ...
    this.animator.startAnimation(
      "labelOpacity",
      this.simulator.getCurrentLabelOpacity(),
    );
    this.animator.startAnimation(
      "labelOpacityHover",
      this.simulator.getCurrentLabelOpacity(),
