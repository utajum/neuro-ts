...
const validColors = z.union([
  ...
  z
    ...
    .refine(
      (val) =>
        val.match(/^#([0-9A-Fa-f]{3}){1,2}$|^--?[_a-zA-Z]+[_a-zA-Z0-9-]*$/),
      {
        message:
          "Invalid color format, expected a hex color (e.g. #RRGGBB) or a CSS variable (e.g. --my-color)",
      },
    ),
]);
...
const percentageSchema = z.union([
  ...
  z
    ...
    .refine((val) => val.match(/^\d+\.?\d?\d?%?$/), {
      message:
        'Invalid percentage format, expected a string in the format "XX%"',
    })
...
export const nodeStyleSchema = z.object({
  ...
  shapeSize: z
    .number()
    .gt(0, "Shape size may not be zero or negative")
    .optional(),
  /**
  ...
  shapePoints: z
    .number()
    .min(2, "The number of points for the shape may not be smaller than 2")
    .optional(),
  /**
...
export const nodeVisitedStyle = {
  shapeColor: "nodeColorVisited" as NodeColorType,
};

export const nodeCurrentStyle = {
  shapeColor: "nodeColorCurrent" as NodeColorType,
};

export const nodeUnresolvedStyle = {
  shapeColor: "nodeColorUnresolved" as NodeColorType,
};
