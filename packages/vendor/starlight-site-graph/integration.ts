...
import {
  ...
  starlightSiteGraphConfigSchema,
  validateConfig,
} from "./config";
...
export default defineIntegration({
  ...
  setup({ name, options }) {
    if (!options.starlight) {
      ...
      options = {
        ...validateConfig(starlightSiteGraphConfig, options),
        starlight: false,
      };
    }
      ...
      hooks: {
        "astro:config:setup": async (args) => {
          ...
          if (!settings.sitemapConfig.contentRoot) {
            // Vendored fix: `trimSlashes(config.srcDir.pathname)` stripped the
            // leading `/` from absolute POSIX paths, producing
            // `home/user/...` instead of `/home/user/...`. Use the
            // `fileURLToPath` helper which gives a real absolute path.
            const srcRoot = fileURLToPath(config.srcDir);
            settings.sitemapConfig.contentRoot = settings.starlight
              ? `${srcRoot.replace(/[\/\\]+$/, "")}/content/docs`
              : `${srcRoot.replace(/[\/\\]+$/, "")}/pages`;
          }
          ...
          if (!config.prefetch) {
            log(logger, "warn", "Astro `prefetch` is disabled", {
              impact:
                "The site graph requires prefetching to function correctly; it has been auto-enabled",
          ...
          if (settings.sitemapConfig.sitemap === undefined) {
            ...
            if (command === "dev" || command === "build") {
              ...
              try {
                ...
                log(
                  logger,
                  "error",
                  "Sitemap generation from Markdown failed",
                  {
              ...
              if (!sitemapGenerationFailed) {
                try {
                  await builder.addMDContentFolder(
                    settings.sitemapConfig.contentRoot,
                    settings.sitemapConfig.pageInclusionRules,
                  );
                  settings.sitemapConfig.sitemap = builder
                    .process()
                    .toSitemap();
                  log(
                    logger,
                    "info",
                    "Successfully generated sitemap from Markdown content",
                  );
                } catch (e) {
                  sitemapGenerationFailed = true;
            ...
            settings.sitemapConfig.sitemap = processSitemap(
              settings.sitemapConfig.sitemap,
              settings,
            );
          }
          ...
          if (sitemapGenerationFailed) {
            log(
              logger,
              "warn",
              `Empty sitemap was generated, the graph visualization will be empty`,
            );
          } else if (command === "dev") {
            const nodeCount = Object.keys(
              settings.sitemapConfig.sitemap ?? {},
            ).length;
            const linkCount = Object.values(
              settings.sitemapConfig.sitemap ?? {},
            ).reduce((sum, node) => {
            ...
            log(
              logger,
              "info",
              `Sitemap with ${nodeCount} nodes and ${linkCount} links generated and injected into virtual module`,
            );
          }
        ...
        "astro:build:done": async (args) => {
          ...
          if (!outputPath) {
            log(logger, "warn", "Could not determine output directory", {
              impact:
                "Metadata and links extracted from HTML will be missing from the graph",
          ...
          if (!sitemapProvided) {
            ...
            try {
              ...
              log(
                logger,
                "warn",
                "HTML link extraction failed. Falling back to Markdown-only data",
                {
          ...
          try {
            ...
            const linkCount = Object.values(
              settings.sitemapConfig.sitemap ?? {},
            ).reduce((sum, node) => sum + (node.links?.length ?? 0), 0);
