...
export class SiteMapBuilder {
  ...
  frontmatterData: Map<
    string,
    { data?: PageSiteGraphFrontmatter } & { slug?: string }
  > = new Map();
  ...
  constructor(private config: SitemapConfig) {
    ...
    this.explicitNameAssociations = new Map(
      Object.entries(this.config.pageTitles).map(([k, v]) => [
        setSlashes(k, true, this.addTrailingSlash),
        v,
      ]),
    );
  }
  ...
  async addHTMLContentFolder(folder: string, patterns: string[] = []) {
    for await (const filePath of walk(folder)) {
      ...
      if (path.extname(filePath) === ".html") {
        const relativePath = ensureLeadingSlash(
          path.relative(folder, filePath).replace(/\\/g, "/"),
        );
  ...
  async addHTMLContent(filePath: string, folderPath: string) {
      ...
      while (current && current.type === "tag") {
        if (
          ...
          current.attribs?.["class"]
            ?.split(" ")
            .some((c: string) => excludedSelectors.has("." + c))
        ) {
          return false;
        }
    ...
    for (const link of includedLinks) {
      ...
      if (href) {
        ...
        if (text.length) {
          this.implicitNameAssociations.set(href, [
            ...(this.implicitNameAssociations.get(href) ?? []),
            text,
          ]);
        }
      }
    ...
    const currentLinkRules = (
      this.frontmatterData.get(linkPath)?.data?.sitemap?.linkInclusionRules ??
      []
  ...
  async addMDContent(filePath: string, folderPath: string) {
    const content = await fs.promises.readFile(filePath, "utf8");
    const frontmatter = matter(content) as unknown as {
      data: PageSiteGraphFrontmatter & { slug?: string };
    };
    ...
    // Otherwise, re-create the Astro slug from the file path
      ...
      linkPath = setSlashes(
        this.getLinkPath(filePath, folderPath, this.basePath),
        true,
    ...
    if (frontmatter.data) {
      ...
      if (frontmatter.data.sitemap?.pageTitle) {
        this.explicitNameAssociations.set(
          linkPath,
          frontmatter.data.sitemap.pageTitle,
        );
      } else if (
        frontmatter.data.title &&
        !this.explicitNameAssociations.has(linkPath)
      ) {
  ...
  private resolveLinkName(linkPath: string) {
    ...
    if (name === undefined) {
      if (this.config.pageTitleFallbackStrategy === "linkText") {
        name = getMostCommonItem([
          ...(this.implicitNameAssociations.get(linkPath) ?? []),
        ]);
      }
    ...
    return Object.fromEntries(
      Array.from(this.map.entries()).map(([link, entry]) => {
        const backlinks = entry.backlinks.size
          ? [...entry.backlinks].sort((x, y) =>
              this.resolveLinkName(x).localeCompare(this.resolveLinkName(y)),
            )
          : undefined;
        ...
        return [
          entry.linkPath,
          {
            ...
            tags: entry.tags.size
              ? [...entry.tags].map(ensureLeadingPound)
              : undefined,
  ...
  private resolveLink(current: string, link: string, links: Set<string>) {
    if (!(link.startsWith("http") || link.startsWith("mailto:"))) {
      ...
      link = setSlashes(
        slugifyPath(link.split("#")[0]!.replace(/\\/g, "/")),
        true,
        this.addTrailingSlash,
      );
      if (link !== current) {
      ...
      if (!link.includes("#"))
        link = setSlashes(link, false, this.addTrailingSlash);
