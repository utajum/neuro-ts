/**
 * `neuro-js-proxy` - a Web-standard fetch handler that fulfils the
 * `proxyUrl` contract documented at https://neuro-js.dev/guides/proxy-contract/.
 *
  ...
  return async function handler(req: Request): Promise<Response> {
    ...
    try {
      const completion = await openai.chat.completions.create({
        model: body.model || config.defaultModel || 'gpt-4o-mini',
