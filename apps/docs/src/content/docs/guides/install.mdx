...
## Server-side subpaths

The same npm install also exposes two server-only subpaths for building a
proxy or token-issuer endpoint. They are excluded from the browser bundle
so the OpenAI SDK never reaches the client:
...
See [Custom proxy contract](/guides/proxy-contract/) and
[Deploy the proxy](/guides/deploy-proxy/) for full setup.
